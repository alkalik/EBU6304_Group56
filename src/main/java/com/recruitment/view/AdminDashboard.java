...
public class AdminDashboard extends JFrame {
    ...
    private static final Font F_ICON_BODY = iconCapableFont(Font.PLAIN, 13);
    private static final Font F_ICON_BOLD = iconCapableFont(Font.BOLD, 13);

    private Runnable refreshAllJobsAction;
    private Runnable refreshAllAppsAction;
    ...
    private void initUI() {
        ...
        tabs.addChangeListener(e -> {
            if (e.getSource() instanceof JTabbedPane) {
                ...
                if (idx == 2 && refreshAllJobsAction != null) refreshAllJobsAction.run();
                else if (idx == 3 && refreshAllAppsAction != null) refreshAllAppsAction.run();
            }
        });
    ...
    private JPanel createWorkloadPanel() {
        ...
        JButton detailBtn = ghost("User  TA Details");
    ...
    private JPanel createUsersPanel() {
        ...
        searchBar.add(new JLabel("Search:")); searchBar.add(kwF); searchBar.add(sb); searchBar.add(cb); searchBar.add(resLbl);
    ...
    private JPanel createAllJobsPanel() {
        ...
        refreshAllJobsAction = refreshAll;
        JPanel btns = btnRow();
        JButton refresh = ghost("↻  Refresh");
        refresh.addActionListener(e -> refreshAll.run());
        JButton del = pill("Delete Job", C_DANGER);
        del.addActionListener(e -> {
            int r = table.getSelectedRow();
            if (r < 0) { warn("Please select a job."); return; }
            ...
            if (confirm("Permanently delete this job?")) {
                if (jobService.deleteJob(jobId)) {
                    ...
                    warn("Job not found. It may have been deleted already — refreshing list.");
                    refreshAll.run();
                }
            }
        });
    ...
    private JPanel createAllApplicationsPanel() {
        ...
        refreshAllAppsAction = refreshAll;
        JPanel btns = btnRow();
        JButton refresh = ghost("↻  Refresh");
        refresh.addActionListener(e -> refreshAll.run());
    ...
    static JButton pill(String t, Color bg) {
        JButton b = new JButton(t); b.setFont(buttonFont(t, F_BOLD, F_ICON_BOLD)); b.setBackground(bg); b.setForeground(Color.WHITE);
    ...
    static JButton ghost(String t) {
        JButton b = new JButton(t); b.setFont(buttonFont(t, F_BODY, F_ICON_BODY));
        b.setFocusPainted(false); b.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); return b;
    }

    private static Font buttonFont(String text, Font normalFont, Font iconFont) {
        return containsIconCharacter(text) ? iconFont : normalFont;
    }

    private static boolean containsIconCharacter(String text) {
        return text != null && text.codePoints().anyMatch(cp ->
                cp > 0x2600 || Character.getType(cp) == Character.OTHER_SYMBOL);
    }

    private static Font iconCapableFont(int style, int size) {
        String[] preferredFonts = {"Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji", "Apple Color Emoji", Font.DIALOG};
